...
function log_kdm_ant_switch_metrics
{
	if [ "${ORIENTATION_REMAIN}" -ne 0 ]; then
		logStr="wifiKDM:AntSwitch:fgtracking=false;DV;1,key=orient_total;DV;1,Counter=${ORIENTATION_REMAIN};CT;1,unit=count;DV;1,metadata=!{"d"#{"type"#"fail"$"reason"#"remain"}};DV;1:NR"
		log -t $METRICSTAG -v $logStr
	fi

	if [ "${ORIENTATION_DISCONNECT}" -ne 0 ]; then
		logStr="wifiKDM:AntSwitch:fgtracking=false;DV;1,key=orient_total;DV;1,Counter=${ORIENTATION_DISCONNECT};CT;1,unit=count;DV;1,metadata=!{"d"#{"type"#"fail"$"reason"#"disconnect"}};DV;1:NR"
		log -t $METRICSTAG -v $logStr
	fi

	if [ "${ORIENTATION_SCAN}" -ne 0 ]; then
		logStr="wifiKDM:AntSwitch:fgtracking=false;DV;1,key=orient_total;DV;1,Counter=${ORIENTATION_SCAN};CT;1,unit=count;DV;1,metadata=!{"d"#{"type"#"fail"$"reason"#"scan"}};DV;1:NR"
		log -t $METRICSTAG -v $logStr
	fi

	if [ "${ORIENTATION_STRONG}" -ne 0 ]; then
		logStr="wifiKDM:AntSwitch:fgtracking=false;DV;1,key=orient_total;DV;1,Counter=${ORIENTATION_STRONG};CT;1,unit=count;DV;1,metadata=!{"d"#{"type"#"success"$"reason"#"strong"}};DV;1:NR"
		log -t $METRICSTAG -v $logStr
	fi

	if [ "${ORIENTATION_SUCCESS}" -ne 0 ]; then
		logStr="wifiKDM:AntSwitch:fgtracking=false;DV;1,key=orient_total;DV;1,Counter=${ORIENTATION_SUCCESS};CT;1,unit=count;DV;1,metadata=!{"d"#{"type"#"success"$"reason"#"success"}};DV;1:NR"
		log -t $METRICSTAG -v $logStr
	fi

	if [ "${ORIENTATION_SWITCHBACK}" -ne 0 ]; then
		logStr="wifiKDM:AntSwitch:fgtracking=false;DV;1,key=orient_total;DV;1,Counter=${ORIENTATION_SWITCHBACK};CT;1,unit=count;DV;1,metadata=!{"d"#{"type"#"success"$"reason"#"switchback"}};DV;1:NR"
		log -t $METRICSTAG -v $logStr
	fi

	if [ "${SCAN_TOTAL}" -ne 0 ]; then
		logStr="wifiKDM:AntSwitch:fgtracking=false;DV;1,key=scan_total;DV;1,Counter=`expr ${SCAN_TOTAL} - ${SCAN_CURRENT_NORSP} - ${SCAN_TARGET_NORSP}`;CT;1,unit=count;DV;1,metadata=!{"d"#{"reason"#"normal"}};DV;1:NR"
		log -t $METRICSTAG -v $logStr
	fi

	if [ "${SCAN_CURRENT_NORSP}" -ne 0 ]; then
		logStr="wifiKDM:AntSwitch:fgtracking=false;DV;1,key=scan_total;DV;1,Counter=${SCAN_CURRENT_NORSP};CT;1,unit=count;DV;1,metadata=!{"d"#{"reason"#"current_noresp"}};DV;1:NR"
		log -t $METRICSTAG -v $logStr
	fi

	if [ "${SCAN_TARGET_NORSP}" -ne 0 ]; then
		logStr="wifiKDM:AntSwitch:fgtracking=false;DV;1,key=scan_total;DV;1,Counter=${SCAN_TARGET_NORSP};CT;1,unit=count;DV;1,metadata=!{"d"#{"reason"#"target_noresp"}};DV;1:NR"
		log -t $METRICSTAG -v $logStr
	fi

	if [ "${SWITCH_BTBUSY}" -ne 0 ]; then
		logStr="wifiKDM:AntSwitch:fgtracking=false;DV;1,key=switch_total;DV;1,Counter=${SWITCH_BTBUSY};CT;1,unit=count;DV;1,metadata=!{"d"#{"reason"#"btbusy"}};DV;1:NR"
		log -t $METRICSTAG -v $logStr
	fi

	if [ "${SWITCH_TIMEOUT}" -ne 0 ]; then
		logStr="wifiKDM:AntSwitch:fgtracking=false;DV;1,key=switch_total;DV;1,Counter=${SWITCH_TIMEOUT};CT;1,unit=count;DV;1,metadata=!{"d"#{"reason"#"timeout"}};DV;1:NR"
		log -t $METRICSTAG -v $logStr
	fi

	if [ "${SWITCH_WRONGANT}" -ne 0 ]; then
		logStr="wifiKDM:AntSwitch:fgtracking=false;DV;1,key=switch_total;DV;1,Counter=${SWITCH_WRONGANT};CT;1,unit=count;DV;1,metadata=!{"d"#{"reason"#"wrongant"}};DV;1:NR"
		log -t $METRICSTAG -v $logStr
	fi

	if [ "${SWITCH_OTHER}" -ne 0 ]; then
		logStr="wifiKDM:AntSwitch:fgtracking=false;DV;1,key=switch_total;DV;1,Counter=${SWITCH_OTHER};CT;1,unit=count;DV;1,metadata=!{"d"#{"reason"#"other"}};DV;1:NR"
		log -t $METRICSTAG -v $logStr
	fi
}

function wifi_power_metric_str
{
	case $1 in
		chip_active)
			...
			echo "wifiKDM:conn-soc-active:fgtracking=false;DV;1,key=$2;DV;1,Timer=$3;TI;1,unit=ms;DV;1,metadata=!{"d"#{"metadata"#"$4"$"metadata1"#"$5"}};DV;1:HI"
			;;
		host_wakeup)
			# $2: metric key, valid values: wifi, app
			# $3: wake up counter value
			echo "wifiKDM:wifi-num-wakeup-host:fgtracking=false;DV;1,key=$2;DV;1,Counter=$3;CT;1,unit=count;DV;1:NR"
			;;
		*)
			# the default case
			;;
...
function log_wifi_power_metrics
{
	local fw_active_stat=`$IWPRIV $WLAN_INTERFACE driver "fw_active_statistics get"`
	...
	local wifi_on_stat=`$IWPRIV $WLAN_INTERFACE driver "wifi_on_time_statistics get"`
	...
	log -t $METRICSTAG `wifi_power_metric_str chip_active 1 $((fw_active_ms-LAST_FW_ACTIVE_MS)) 0 FW` # firmware active duration while screen off and FW own
	...
	log -t $METRICSTAG `wifi_power_metric_str chip_active 0 $((hw_active_ms-LAST_HW_ACTIVE_MS)) 0 FW` # hardware active duration while screen off and FW own
	LAST_HW_ACTIVE_MS=$hw_active_ms

	if [ $mgmt_wakeup -lt $LAST_MGMT_WAKEUP ]; then
		# The value in driver is cumulative, not impacted by wifi reset. We reach here to handle the case of overflow.
		hw_active_ms=$((unsigned_int_max-LAST_MGMT_WAKEUP+mgmt_wakeup))
		LAST_MGMT_WAKEUP=0 # Reset last value to 0 to report current value directly.
	fi
	log -t $METRICSTAG `wifi_power_metric_str host_wakeup mgmt $((mgmt_wakeup-LAST_MGMT_WAKEUP))` # Wifi wakes up host due to event/mgmt frames, the wake-up statistic data are collected in iwpriv_int_stat_tokens
	...
	log -t $METRICSTAG `wifi_power_metric_str host_wakeup app $((data_wakeup-LAST_DATA_WAKEUP))` # Wifi wakes up host due to receiving APP's data
	...
	log -t $METRICSTAG `wifi_power_metric_str chip_active 2 $((screen_off_ms-LAST_WIFI_ON_SCR_OFF)) 0 DRV` # Wifi on duration during wifi off
...
function run ()
{
	...
	if [ -n "$WLAN_INTERFACE" ]; then
		...
		$IWPRIV $WLAN_INTERFACE driver "ant_switch_test 26" > /dev/null
	fi
}

# Initialize wifi power metrics related parameters before main loop.
